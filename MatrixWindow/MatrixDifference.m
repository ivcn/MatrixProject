...
% Last Modified by GUIDE v2.5 19-Jun-2012 19:11:54
...
k=size(colormap,1);
flag_intens = get(handles.flag_intens, 'Value');
if (flag_intens ~= 0)
    wd = handles.cur_matrix(:,:,c).*k./handles.cur_maxm;
else
    wd = (handles.cur_matrix(:,:,c)+abs(handles.cur_minm)).*k./(abs(handles.cur_minm)+handles.cur_maxm); 
end
     ...
     if sm(cursm)~=0    
         ...
         for ty = 1:ny
            for tx = 1:mx
                ...
                if (mod(tx*ty,25)==0)
                    ['sm ' int2str(cursm) ' row# ' int2str(tx*ty)]
                end    
                ...
                %
                wrowrez = [wrowrez; B]; 
                wrow_without_trend = [wrow_without_trend; E]; 
            end
         end
       
        l1 = size(wrow_without_trend,2);
...
%         guidata(hObject, handles);
     ...
     handles.minm_en = min(min(min(wdata)));
     guidata(hObject, handles);
 ...
 %  T = wdata_wt(:,:,1) ;
%   TT = cell2mat(handles.mat_intens(5));
 %  TT = TT(:,:,1);
...
[l,wrow, nullintervals, mat_intens, masks]=ExtractRows(handles.date, dni,'D:\Veivlet tufanov\ExtractMatrix',nx, ny,sf, st, ol, or, ot, ob, sm, polosi);
...
% --- Executes on button press in flag_intens.
function flag_intens_Callback(hObject, eventdata, handles)
% hObject    handle to flag_intens (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of flag_intens
% handles.cur_matrix = handles.mat_intens(:,:,:,5);
% guidata(hObject, handles); 
% handles.cur_maxm = handles.maxm_intens(5);
% guidata(hObject, handles); 
...
handles.cur_minm = handles.minm_en;
handles.cur_maxm = handles.maxm(5);
guidata(hObject, handles);
...
handles.cur_maxm = handles.maxm_wt(5);
handles.cur_minm = handles.minm_wt;
guidata(hObject, handles); 
'trend_matrix'
