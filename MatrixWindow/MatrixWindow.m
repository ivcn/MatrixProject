        ...
        for j = 1:1:nx*ny
            ...
            if flagCheb ~= 0  % Chebishev
                if (flagfish~=0) % fisher
                    ...
                    [F,B] = Approx(ff,l,-1,-1,-1);%F-тренд, B - остаток
                end
...
if(get(handles.checkbox24,'Value')~=0)%информация о фильтре добавляется только когда фильтр включен
...
%     size(ffsum)
    ...
    %в цикле обрабатываются данные для каждого супермодуля
    for cursm = 1:4
        if numsm > 1
            subplot(3,2,cursm);
        end    
...
%              size(ff)
            ...
            %если у нас больше одного супермодуля
            %что-то непонятное...
            if numsm > 1
                hold on;
    ...
    %Обработка суммы по всем супермодулям
    if numsm > 1
        subplot(3,2,5:6);
    end    
    
    ff = sum(ffsum);%сумма по супермодулям
    n=size(ff,2);
    handles.sig(5) = std(ff);
    guidata(hObject, handles);

    flagvibr = get(handles.vibr, 'Value');
    if (flagvibr ~= 0)    % удаляем выбросы, если нужно
       predel = eval(get(handles.predel, 'String'));
       ff=UdalVibr(ff, predel);
    end
...
%     end  
    ...
    %здесь делается что-то с нулевыми интервалами
    for i=1:4    
        if sm(i)~=0
    ...
    %выводим график суммарного сигнала
...
%выводим в заголовок окна информацию о фильтре
...
% Obrabotka summi
    ...
    if numsm > 1
        ...
        title(sprintf(strcat('Энергия ряда\n',figname)));
    end
...
if numsm > 1
    ...
    title(sprintf(strcat('Полная энергия матричного ряда\n',figname)));
end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    %извлекаем данные из ячейки wrow и кладем их в ffsum
    handles.B = [];
    for cursm = 1:4
        if sm(cursm)~=0   
            ff = cell2mat(handles.wrow(cursm));
    ...
    ff = sum(ffsum);%в массив ff теперь кладем сумму по супермодулям
...
%     guidata(hObject, handles);
    ...
    flagvibr = get(handles.vibr, 'Value');
    if (flagvibr ~= 0)    %Удаляем выбросы перед обработкой, если нужно.
        predel = eval(get(handles.predel, 'String'));
        ff=UdalVibr(ff, predel);
    end
    ...
    % в F будет лежать тренд, в B - ряд без тренда
    %------------------------------------------------


    %-----здесь мы удаляем выбросы с обработанного ряда----------------
    flagmontrend = get(handles.montrend, 'Value');
    if (flagmontrend ~= 0) %если тренд надо убрать
        flagsigma = get(handles.sigma, 'Value');

        ff = handles.B(5,:);%в массив ff кладем ряд без тренда
        n=size(ff,2);
    ...
    flagw = get(handles.flagw, 'Value');%вывести W^2
    flagsigma = get(handles.sigma, 'Value');%расчет в сигмах
    %if (flagssa == 0)   % Chebishev
    %    flagfish = get(handles.fisher, 'Value');
    %    if (flagfish == 0) % fisher
...
%если фильтр применять не надо, выходим
